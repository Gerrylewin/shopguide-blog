...
- Reader thumbs up/down is a floating bottom-left widget on blog post routes only (`BlogPostVote` from `app/blog/[...slug]/page.tsx`, `/api/blog-vote`, Cloudflare D1). If D1 env vars are unset, the widget does not render. The vote summary page is `/admin/blog-votes`; access is gated by matching `BLOG_VOTES_ADMIN_SECRET` as a URL query token—share that URL with teammates who need the dashboard.
